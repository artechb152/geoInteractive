...
Turns a reference image into pixel-accurate code in this repo, without scope creep or drift from the visual reference. This is the same discipline documented in root `CLAUDE.md` under "UI Fidelity Rules," generalized so it applies to any mockup — the landing page, a single lesson scene, a widget — not just `design/mockup.png`.
