...
import type { View } from './OnboardingScene';

/**
 * WebP frame-sequence player for the mobility/obstacles/cover/concealment
 * accordion — same technology as topic-01/SceneOnboardingFramePlayer.tsx
 * (see that file for the full rationale). Frames are drawn onto a single
 * <canvas> (never swapped DOM elements) so stepping through the sequence
 * never flickers or shows an empty frame. Timing is computed from
 * performance.now()/requestAnimationFrame against a target duration, so a
 * route through several transition clips still lands inside its overall
 * time budget.
 */

const ASSET_BASE = '/assets/scene-onboarding/topic-04';

const STEPS_ORDER: View[] = ['access', 'obstacles', 'cover', 'concealment'];
const STEP_INDEX: Record<View, number> = { access: 0, obstacles: 1, cover: 2, concealment: 3 };

const TRANSITION_IDS = ['transition-1-2', 'transition-2-3', 'transition-3-4'] as const;
type TransitionId = (typeof TRANSITION_IDS)[number];

// Extracted via ffmpeg (fps=12 from the 24fps/4s source shots) — same
// convention as topic-01/02. Update if the source footage or extraction
// rate ever changes.
...
const STATE_IMAGE_PATH: Record<View, string> = {
  access: `${ASSET_BASE}/states/state-1-access.webp`,
  obstacles: `${ASSET_BASE}/states/state-2-obstacles.webp`,
  cover: `${ASSET_BASE}/states/state-3-cover.webp`,
  concealment: `${ASSET_BASE}/states/state-4-concealment.webp`,
};
...
// Same pacing as topic-01/02: a direct (single-segment) hop targets
// ~1.2-1.4s; a multi-segment route targets ~1.5s TOTAL, split across its
// segments below — not per segment.
...
export default function SceneOnboardingFramePlayer({ targetState }: { targetState: View }) {
  ...
  const settledStepRef = useRef<View>(targetState);
  // Final destination of the route currently playing (null when idle).
  const currentRouteDestRef = useRef<View | null>(null);
  ...
  const startRoute = (fromStep: View, toStep: View) => {
  ...
  const handleTargetChange = (newTarget: View) => {
