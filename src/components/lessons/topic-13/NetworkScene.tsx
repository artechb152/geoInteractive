...
export function NetworkScene() {
  ...
  return (
    <section id="scene-network" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {/* Targeting insight */}
      <div className="surface-elevated p-5 rounded-2xl mb-12 border-r-4 border-r-accent">
        <div className="flex gap-3 items-start">
        ...
        {nodes.map((n) => {
          ...
          return (
            <g key={n.id} onClick={() => !isEndpoint && onToggle(n.id)} style={{ cursor: isEndpoint ? 'default' : 'pointer' }}>
              ...
              {/* Critical badge */}
              {n.isCritical && !isDisabled && (
                <text x={n.x + 2.5} y={n.y - 2} className="fill-status-warn font-mono font-bold" fontSize="2">
                  ⚠
                </text>
              )}
            </g>
      ...
      >
        ...
        {showBuffers && threats.map((t) => (
          <g key={t.id} className="pointer-events-none">
            ...
            {/* Range label */}
            <text x={t.x + t.range - 2} y={t.y - 1} className="fill-status-danger font-mono" fontSize="1.6" paintOrder="stroke" stroke="#0a0f1a" strokeWidth="0.6" strokeLinejoin="round">
              Kill Box
            </text>
          </g>
        ))}
