...
isActive
 ...
 <p className="text-sm text-fg-muted leading-relaxed text-pretty mb-4">
  <strong className="text-fg">מהו ראסטר בפועל:</strong> הראסטר הוא הדרך שבה מחשב מייצג משטח רציף — הוא מחלק את השטח לרשת של תאים (פיקסלים), ולכל תא נותן ערך מספרי אחד: גובה, טמפרטורה, צפיפות צמחייה. ככל שהתא קטן יותר, כך הרזולוציה גבוהה יותר. בצבא, ראסטר מרכזי הוא DTM — מודל פני הקרקע הספרתי (גובה הקרקע החשופה בלבד, ללא צמחייה ומבנים) — משמש לחישובי קווי ראייה, נסתרות ושיפועים.
 </p>
...
draggable={false}
 ...
 <p className="text-xs text-fg-muted leading-relaxed text-pretty mt-2">
  <strong className="text-fg">זהו אורתופוטו</strong> — תמונה (אווירית או לוויינית) שעברה תיקון גיאומטרי (אורתורקטיפיקציה) שמסיר את עיוותי זווית הצילום וגובה השטח, כך שכל פיקסל יושב במיקום האמיתי שלו. בזכות זה אפשר למדוד עליו מרחקים ולהשכיב עליו שכבות וקטוריות בדיוק — בתצלום רגיל (לא מתוקן) המרחקים משתבשים ליד הקצוות.
 </p>
 </figcaption>
 </figure>
