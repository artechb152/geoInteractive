...
return <circle key={i} cx={x} cy={y} r="0.2" className="fill-white" opacity={0.4 + (i % 3) * 0.2} />;
 ...
 <text x="3" y={m.y - 0.5} className="fill-white font-display font-bold" fontSize="2" opacity="0.7"
        paintOrder="stroke"
        stroke="#ffffff"
        strokeWidth="0.9"
...
style={{ color: isActive ? `var(--${platform.color.replace('text-', '')})` : undefined }}
 />
 <text x={xPos} y={p.y + 3} textAnchor="middle" className={cn('font-display font-bold font-bold', isActive ? platform.color : 'fill-white/60')} fontSize="1.6"
        paintOrder="stroke"
        stroke="#ffffff"
        strokeWidth="0.9"
        strokeLinejoin="round"
...
return (
 ...
 <div className="text-sm font-display font-semibold text-fg-muted mb-0.5 tracking-wider">{label}</div>
