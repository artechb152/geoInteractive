  ...
  return (
    <section id="scene-scale" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <SceneHeader
        ...
        title={
          <>
            המספר הקטן הזה — <span className="gradient-text">משנה הכל</span>
          </>
        }
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="pt-3 border-t border-border-subtle">
                <div className="text-[10px] font-mono text-fg-dim mb-1 tracking-widest uppercase">משימה אופיינית</div>
      ...
      <svg viewBox="0 0 100 75" className="w-full h-full select-none" preserveAspectRatio="xMidYMid slice">
        ...
        {detailLevel === 'low' && [
          ...
          <g key={i}>
            ...
            <text x={p.x + p.w / 2} y={p.y + p.h / 2 + 0.5} textAnchor="middle" className="fill-fg-dim text-[2.5px] font-mono font-bold tracking-tighter">{p.label}</text>
          </g>
        ))}

        {/* Main Road */}
        <path 
          d={detailLevel === 'low' ? "M0 60 L 100 50" : "M0 60 Q 30 58 50 56 T 100 50"} 
          fill="none" 
          className="stroke-accent/60" 
          strokeWidth={detailLevel === 'high' ? 0.4 : 0.8} 
        />
        ...
        <g transform="translate(68, 68)">
          ...
          <text x="0" y="-1.5" className="fill-fg-dim text-[2px] font-mono">0</text>
          <text x="22" y="-1.5" textAnchor="end" className="fill-fg-dim text-[2px] font-mono">
...
function ProjectionCallout() {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="mt-8 surface-elevated p-6 border-r-4 border-r-accent-cool rounded-xl"
    >
      <div className="flex gap-4 items-start">
        <Icon name="globe" size={24} className="text-accent-cool shrink-0 mt-0.5" />
        <div className="flex-1">
          <div className="text-[10px] font-mono text-accent-cool mb-1 tracking-widest uppercase font-bold">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <ProjectionTile 
              ...
              scope="היטל צבאי מקומי" 
              tradeoff="המדויק ביותר לישראל. משמש את צה''ל לניווט, תצפית וירי ארטילרי." 
            />
            <ProjectionTile 
              name="Web Mercator" 
              scope="גוגל מפות (Web)" 
              tradeoff="נוח לניווט עירוני, אבל מעוות שטחים (גרינלנד נראית גדולה מאפריקה)." 
            />
            <ProjectionTile 
              name="UTM" 
              scope="סטנדרט נאט''ו" 
              tradeoff="מחלק את העולם ל-60 רצועות דיוק. חיוני לעבודה עם צבאות זרים." 
            />
          </div>
