...
import type { LayerId } from './OnboardingScene';

/**
 * WebP frame-sequence player for the map-layers accordion — same technology
 * as topic-01/SceneOnboardingFramePlayer.tsx (see that file for the full
 * rationale). Frames are drawn onto a single <canvas> (never swapped DOM
 * elements) so stepping through the sequence never flickers or shows an
 * empty frame. Timing is computed from performance.now()/requestAnimationFrame
 * against a target duration, so a route through several transition clips
 * still lands inside its overall time budget.
 */

const ASSET_BASE = '/assets/scene-onboarding/topic-02';

const STEPS_ORDER: LayerId[] = ['base', 'roads', 'buildings', 'borders', 'ops'];
const STEP_INDEX: Record<LayerId, number> = { base: 0, roads: 1, buildings: 2, borders: 3, ops: 4 };
...
// PLACEHOLDER counts — update once the real footage is extracted via ffmpeg
// (see topic-01's player for the extraction convention: fps=12 from a
// 24fps/4s source shot ⇒ 48 frames per transition). These numbers must match
// however many frame-NNNN.webp files actually land in each transition folder.
...
const STATE_IMAGE_PATH: Record<LayerId, string> = {
  base: `${ASSET_BASE}/states/state-1-base.webp`,
  roads: `${ASSET_BASE}/states/state-2-roads.webp`,
  buildings: `${ASSET_BASE}/states/state-3-buildings.webp`,
  borders: `${ASSET_BASE}/states/state-4-borders.webp`,
  ops: `${ASSET_BASE}/states/state-5-ops.webp`,
};
...
// Same pacing as topic-01: a direct (single-segment) hop targets ~1.2-1.4s; a
// multi-segment route targets ~1.5s TOTAL, split across its segments below.
...
// Same module-level-flag pattern as topic-01's player — read directly inside
// the rAF loop rather than prop-drilled React state.
...
export default function SceneOnboardingFramePlayer({ targetState }: { targetState: LayerId }) {
  ...
  const settledStepRef = useRef<LayerId>(targetState);
  // Final destination of the route currently playing (null when idle).
  const currentRouteDestRef = useRef<LayerId | null>(null);
  // Latest target requested mid-flight; always overwritten, never queued.
  const pendingTargetRef = useRef<LayerId | null>(null);
  ...
  const startRoute = (fromStep: LayerId, toStep: LayerId) => {
  ...
  const handleTargetChange = (newTarget: LayerId) => {
