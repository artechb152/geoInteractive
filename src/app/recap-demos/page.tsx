  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16 w-full">
        <div className="text-[11px] font-display font-semibold tracking-wider text-accent uppercase mb-3">
          דמו · סיכום שיעור
        </div>
        <h1 className="font-display font-bold text-3xl md:text-5xl text-balance leading-tight mb-3 text-accent">
          4 דרכים לסכם את <span className="text-accent">שיעור 1</span> ולתרגל פעיל
        </h1>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {DEMOS.map((d) => (
            <Link
              key={d.id}
              href={d.href}
              className="group rounded-2xl border border-border bg-bg-elevated p-6 hover:border-accent/40 transition-all relative overflow-hidden"
            >
              <div aria-hidden className="absolute -top-12 -end-12 size-32 rounded-full bg-accent/8 blur-3xl pointer-events-none" />
              <div className="relative">
                <div className="text-[11px] font-display font-semibold tracking-wider text-accent uppercase mb-1.5">
                  פורמט {d.id}
                </div>
                <h2 className="font-display font-bold text-xl mb-1.5 text-accent-hover group-hover:text-accent-deep transition-colors">
                  {d.title}
                </h2>
                <p className="text-sm text-fg-muted leading-relaxed mb-5">{d.tagline}</p>
                <div className="inline-flex items-center gap-2 text-sm font-display font-semibold text-accent">
                  <span>פתח דמו</span>
                </div>
              </div>
            </Link>
